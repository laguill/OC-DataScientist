...
app = marimo.App(app_title="P4 Predictions consommations")
...
@app.cell(hide_code=True)
def _(mo):
    mo.md(r"""# Prédictions de la consommation""")
    return
...
def _(mo):
    ...
    L'objectif de cette section est de développer un modèle capable de prédire avec la plus grande précision possible la consommation des bâtiments non résidentiels de la ville de Seattle.
    ...
    (Dans cette partie je cherche à prédire `SiteEnergyUseWN(kBtu)`)
...
def _(targets):
    ...
    target = targets[1]  # SiteEnergyUseWN(kBtu)
    return (target,)
...
@app.cell
def _(features, plt):
    features.plot.kde(subplots=True, sharex=False, layout=(2, 5), figsize=(15, 6))
    plt.show()
    return
...
def _(
    ...
    with mo.persistent_cache(name="01_models_init"):
        results_1 = []
...
def _(
    ...
    with mo.persistent_cache(name="01_models_param_grid_1"):
        results_2 = []
...
def _(mo):
    mo.md(
        r"""Le modèle Linear_regression semble être le plus performant des modèles avec un R² de 0.72. Le R² du dataset de test 0.72."""
    )
    return
...
def _(
    ...
    with mo.persistent_cache(name="01_model_comparison_param_grid2"):
...
def _(mo):
    ...
    Les paramètres obtenus précédemment sont: `'alpha': 0.6905862069`
    ...
    def model_comparison_v2(
        ...
        *,
        ...
        for _model_name, _model_params in tqdm(models_param_grid.items(), desc="Training models"):
            ...
            # Récupère le temps moyen d'ajustement de la validation croisée
            mean_fit_time = model_search.cv_results_["mean_fit_time"].mean()
...
def _(
    ...
    with mo.persistent_cache(name="01_model_comparison_param_grid_3"):
...
def _(
    ...
    with mo.persistent_cache(name="01_shap_values"):
...
def _(mo):
    ...
    Pour faciliter la visualisation des prédictions, j'applique la transformée exponentielle.
    """
    )
    return
...
@app.cell
def _(plt, predictions_v1, predictions_v2, sns, y_test, y_test_v2):
    plt.figure(figsize=(15, 6), dpi=100)

    sns.scatterplot(x=y_test_v2, y=predictions_v2)
    sns.scatterplot(x=y_test, y=predictions_v1)
    ...
    plt.title("Comparaison du model Ridge avec et sans ENERGYSTARScore")
    plt.show()
    return
...
@app.cell(hide_code=True)
def _(X_train_transformed_v2, ridge_model, y_train_v2):
    ridge_model.fit(X_train_transformed_v2, y_train_v2)
    return
...
def _(
    ...
    with mo.persistent_cache(name="01_shap_values_v2"):
        # Calcul des valeurs SHAP pour les instances de test
...
def _(mo):
    ...
    L'utilisation de EnergyStarScore rend le modèle plus performant.
    Les graphiques de shap avec le beeswarm montre l'interet de cette feature.
    En revanche la méthode de calcul du EnergyStarScore nécessite la connaissance de la consommation d'énergie du bâtiment.
    On ne peut pas utiliser cette feature dans le modèle.
    """
    )
    return
